...
public class F6 {
	...
	public static void main14(String[] args) throws IOException {
		...
		File kopo05_f = new File("C:\\Users\\김범주\\Desktop\\한국교통안전공단_전국공영주차장정보2.txt");  // 읽어올 파일 가져올 객체 생성 
		...
		while ((kopo05_readtxt = kopo05_br.readLine()) != null) { // 더이상 읽어올 내용이 없을 때까지 while문으로 반복 
			...
			if ((kopo05_field[3].equals("") || kopo05_field[3] == null)   // kopo05_field[3]는 위도, 위도 정보가 공백이거나 
					|| (kopo05_field[2].equals("") || kopo05_field[2] == null)) { // kopo05_field[2]는 경도, 경도 정보가 공백이면  
			...
			if (kopo05_field[6].isEmpty()) {  // 만약 kopo05_field의 배열 6번째가 비어있다면  
				kopo05_field[6] = kopo05_field[7];   // kopo05_field의 배열 6번째의 값에 kopo05_field의 배열 7번째 값을 넣는다 
			}
			

			System.out.printf("************[%d번째 항목]*****************************\n", kopo05_LineCnt + 1); // 몇번째 항목인지 출력 
			System.out.printf("%s : %s\n", kopo05_field_name[6], kopo05_field[6]);                             // 아까 나눠서 kopo05_field_name에 배열로 넣어준 값을 제목으로해서 kopo05_field[9]의 값인 주소를 넣어줌  
			System.out.printf("%s : %s\n", kopo05_field_name[3], kopo05_field[3]);							// 아까 나눠서 kopo05_field_name에 배열로 넣어준 값을 제목으로해서 kopo05_field[12]의 값인 위도를 넣어줌 
			System.out.printf("%s : %s\n", kopo05_field_name[2], kopo05_field[2]);							// 아까 나눠서 kopo05_field_name에 배열로 넣어준 값을 제목으로해서 kopo05_field[13]의 값인 경도를 넣어줌 
			double kopo05_dist = Math.sqrt(Math.pow(Double.parseDouble(kopo05_field[3]) - kopo05_lat, 2)      
					+ Math.pow(Double.parseDouble(kopo05_field[2]) - kopo05_lng, 2));
			...
			if (kopo05_min > kopo05_dist) {   // kopo05_min이 kopo05_dist보다 크면 
				...
				kopo05_minadd = kopo05_field[6];  //  kopo05_min이 kopo05_dist보다 크면 kopo05_field의 배열 6번째 값을  kopo05_minadd 최소거리 주소 변수에 넣어줌 
				
			} 
			if (kopo05_max < kopo05_dist) {  // kopo05_max가 kopo05_dist보다 작으면 
				kopo05_max = kopo05_dist;   // kopo05_dist값이 kopo05_max값으로 들어가도록 계속해서 비교
				kopo05_maxadd = kopo05_field[6];  //  kopo05_max이 kopo05_dist보다 작으면 kopo05_field의 배열 6번째 값을  kopo05_maxadd 최대거리 주소 변수에 넣어줌 
